...
class Bottleneck(nn.Module):
    def __init__(self, in_channels, out_channels, hidden_dim, num_heads=1, use_nlg=True):
        ...
        self.att = nn.Sequential(
            ...
            NLG(in_channels, out_channels, hidden_dim, num_heads) if use_nlg else CBR(in_channels, out_channels, 3, 1, 1)
        )
...
class DGUnet(BaseModel):
    def __init__(self, in_channels, num_classes, mid_channels=32, depth=4, hidden_dim=None, num_heads=1, use_dg=True, use_nlg=True):
    ...
    def build_stack(self, in_channels, mid_channels, depth, hidden_dim):
        if depth <= 0:
            return Bottleneck(in_channels, mid_channels, hidden_dim, self.num_heads, self.use_nlg)
        
        return ModuleStack(
            EncoderBlock(in_channels, mid_channels),
            self.build_stack(mid_channels, mid_channels * 2, depth - 1, hidden_dim),
            DecoderBlock(mid_channels * 2, mid_channels) if self.use_dg else CatDecoderBlock(mid_channels * 2, mid_channels)
        )
