...
from model.dgunet import DGUnet
...
filter_func = lambda model_name, iteration: 'dgunet' in model_name.lower() and 'DCA1_300' in iteration

# traverse_and_import(base_directory, filter_func)
# exit(0)

sw = SummaryWriter('exp/TEST/vision')
model = DGUnet(3, 1, num_heads=2).to(device)
model.load_state_dict(torch.load(r'exp/KFold/info/DGUnet/DCA1_300_300_000/checkpoint/fold_0_best.pt')['model'])
model.eval()
